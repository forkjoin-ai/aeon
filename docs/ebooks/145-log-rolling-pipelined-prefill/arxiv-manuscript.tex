...
The omega checker (\texttt{omega-checker.gg}) is a Gnosis topology --- a fork/race/fold program written in the language whose compiler is a fork/race/fold pipeline. The checker verifies the theorems that prove the checker's fold is BFT-correct. The compiler verifies the checker's $\beta_1$ bounds. The model checker (\S10) verifies the compiler's temporal properties. Each layer is checked by the layer below it, and the bottom layer checks the top. The structure is not circular --- it is a fern. Each frond repeats the branching pattern of the whole, and the whole is the pattern.
...
\subsubsection{15.2 The American Frontier}\label{american-frontier}
