...
dialogue converges toward mutual understanding. Section 15 further
...
same homology class, same Betti numbers, same energy laws (§6.11).'' The
...
exactly as in the Worthington whip (§6.9). At the snap point, the field
...
(§6.15), then finding it in belief systems is not an analogy -- it is an
...
The opening discussion (section 1) mapped four biological systems to
...
identified in section 1 are backed by quantitative separation in the
cited literature.
...
covering space (§3.4). Flags compose: \texttt{RACE\ \ FIN} means
``racing AND final frame.'' The frame reassembler (§3.4) is the covering
...
\subsubsection{6.17.1 Implementation: Temperature-Ensemble MOA on
Cloudflare
Workers}\label{implementation-temperature-ensemble-moa-on-cloudflare-workers}

The Glossolalia Engine is deployed as a temperature-ensemble Mixture of
Agents running a real transformer on Cloudflare Workers -- edge
serverless with no GPU and no external inference API. Cloudflare Workers
pricing is request-based: the free tier provides 100,000 requests per
day; the paid tier (\$5/month) provides 10 million requests per month
with \$0.50 per additional million. KV reads (32 per token per forward
pass) are 10 million per day on the free tier. At these rates the
per-token cost of inference is negligibly small -- orders of magnitude
below any hosted LLM API.

\textbf{The model.} SmolLM2-360M-Instruct (HuggingFace), a 360 million
parameter language model. 32 transformer layers, 960 hidden dimension,
15 attention heads, five KV heads (grouped query attention), SwiGLU MLP.
Quantized to Q8\_0 (8-bit) and stored as 385MB across 32 KV entries in
Cloudflare's key-value store.

\textbf{The inference path.} BPE tokenize the prompt. Feed each token
through all 32 layers sequentially by streaming: load one layer
(\textasciitilde10MB Q8\_0) from KV, dequantize Q8\_0\(\to\)F32
(\textasciitilde38MB working memory), run attention + MLP, discard the
weights, load the next layer. After the final layer, apply RMSNorm and
project through the lm\_head (tied with embedding weights) to produce
49,152 logit values. The entire 385MB model runs in 128MB of worker
memory by sharding across time instead of space -- loading one layer,
processing, discarding, loading the next. This is the inverse of the
standard approach (load all weights into VRAM or shard across GPUs);
here the shard dimension is temporal, not spatial.

\textbf{The MOA (fork/race/fold).} The fork/race/fold primitive from §2
is instantiated at inference time over the logit distribution:

\begin{itemize}
\tightlist
\item
  \emph{Fork.} Take the raw logits from the single forward pass. Each
  agent applies a different temperature: agent 1 at \(T = 0.4\), agent 2
  at \(T = 1.0\), and so on. Low temperature concentrates probability
  mass on likely tokens; high temperature spreads it across the
  vocabulary. This produces genuinely different probability
  distributions from the same underlying model output.
  \(\beta_1 = k - 1\) where \(k\) is the agent count.
\item
  \emph{Race.} Filter out any agent whose logit vector contains NaN or
  Inf -- the numerical sanity gate. Agents that survive race proceed to
  fold.
\item
  \emph{Fold.} Deficit-weighted merge. Compute each agent's L2
  divergence from the mean distribution. Agents that diverge more
  receive higher weight. This is the semiotic deficit principle
  (THM-SEMIOTIC-ERASURE): the fold from \(k\) distributions to one
  erases \(k - 1\) paths. Deficit weighting minimizes that erasure by
  preserving the most information-rich -- most divergent --
  perspectives. The Bule number \(\Delta_\beta\) of the fold is
  \((k - 1)\), and the deficit-weighted strategy minimizes the
  information loss within that budget.
\item
  \emph{Sample.} Draw one token from the merged distribution with
  top-\(K\)/top-\(P\) filtering.
\end{itemize}

\textbf{Why temperature-ensemble and not depth-ensemble.} The original
design forked agents at different layer depths -- early layers for
syntax, late layers for semantics. The implementation revealed why this
fails: \texttt{computeLogits} applies the final RMSNorm and lm\_head
projection, which are calibrated for the hidden state from layer 32.
Intermediate hidden states from layer two or eight produce garbage
logits because the norm weights and projection expect the full-depth
representation. Temperature-ensemble avoids this by running the complete
forward pass once and forking only at the logit level, where all agents
share a valid hidden state.

\textbf{Known techniques.} Temperature ensembling (running multiple
temperatures, blending) is standard ML. Quantized inference (Q8\_0) is
standard in the llama.cpp ecosystem. Top-\(K\)/top-\(P\) sampling is
standard decoding. RMSNorm, grouped query attention and SwiGLU are
standard transformer architecture.

\textbf{Novel engineering.} Two engineering contributions are claimed:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \emph{Streaming layer-by-layer inference from KV storage.} Running a
  385MB model in 128MB of memory by treating weight loading as a
  streaming pipeline -- load one layer, dequantize, process, discard,
  load next. The standard approach loads all weights into VRAM or shards
  across GPUs (space-parallel). This shards across time (time-serial).
  The tradeoff is latency for memory: each token requires 32 KV fetches,
  but the working set never exceeds one layer.
\item
  \emph{Full transformer inference on Cloudflare Workers.} A 360 million
  parameter model running in a single JavaScript isolate on edge
  serverless -- a platform designed for serving HTML. The model lives in
  KV (Cloudflare's distributed key-value store) and runs with no GPU and
  no external inference API at negligible per-request cost (\$0.50 per
  million requests on the paid tier). The feasibility depends on the
  streaming approach above: without it, the 385MB model would exceed
  worker memory limits by 3x.
\end{enumerate}

\textbf{Novel theory.} Two theoretical contributions connect the
implementation to the formal framework:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \emph{Semiotic deficit-weighted merging.} The fold step does not use
  uniform weights or confidence-based selection. It computes each
  agent's divergence from consensus and upweights the outliers. The
  formal grounding (§3, THM-SEMIOTIC-MOA-ISOMORPHISM): thought has high
  \(\beta_1\) (many semantic paths), speech has \(\beta_1 = 0\) (one
  token stream). The fold erases \(k - 1\) paths. Deficit weighting
  minimizes that erasure by preserving the most information-rich
  perspectives -- the Bule number applied to decoding.
\item
  \emph{Fork/race/fold as a decoding primitive.} The same algebraic
  structure used for distributed systems (§2), biological systems (§1),
  and queueing theory (§4) is instantiated here as a sampling strategy.
  The provable properties (THM-SEMIOTIC-ERASURE,
  THM-SEMIOTIC-MOA-ISOMORPHISM) apply: the deficit is bounded, the merge
  is associative, and the information loss is quantified.
\end{enumerate}

The combination is what the Glossolalia Engine contributes: a
formally-grounded semiotic MOA running a full streaming transformer on
edge serverless at negligible per-request cost. Each piece exists in
isolation -- temperature ensembling, quantized inference, edge compute,
semiotic theory -- but the integration solves a problem none of them
address individually: running a real LLM with a formal theory of agent
merging on a platform with 128MB of memory and no GPU.
...
\subsection{9. Containing Queueing
...
& Structured MoA & §11.5 & GG-backed sparse routing & Topology-level
expert selection \\
& Glossolalia Engine & §6.17 & Vickrey Table + Daisy Chain &
...
(ground state) & Void walking & §14.5 & Complement distribution &
Immanent self-hosting \\
\end{longtable}
}

The stack reads bottom-up: \emph{from building blocks to bytes on wire,
back into execution, and down into self-verification}. Layer 1 (§10)
verifies modeled primitive properties. Layer 2 (§11) gives a language to
write topologies, checked by layer 1 workflows. Layer 3 (§7) schedules
work through the topology, expressed in layer 2's language. Layer 4 (§8)
puts frames on the wire, carrying layer 3's scheduled work. Layer 5 (§9)
compresses the payload -- actual bytes, actual ratios, actual wire --
using layers below it. Layer 6 (§11.5) applies sparse routing at the
topology level, using layer 2's GGL to define MoA structures and layer
3's scheduler to execute them. Layer 7 (§6.17) instantiates
...
layers 2 and 3. Layer 9 (§14.5) is the ground state: void walking uses
the complement distribution over rejection history to verify the
theorems about void walking itself -- immanent self-hosting.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{companion-tests/artifacts/void-walking-atlas.png}}
...
\textbf{Companion tests for §14.5:} 263 tests across 24 files, 0
failures, 695 assertions. 13 Lean theorems (VoidWalking.lean,
...
cross-attention mediator (§14.5.11) \\
...
\textbf{Companion tests for §14.5:} 281+ tests across 28 files, 0
...
\emph{Quantitative convergence} -- how many dialogue turns suffice, what
is the mixing time, at what rate does the deficit shrink -- is
...
(§14.5.10). The void walking engine was used to verify the theorems
about void walking. The complement distribution measured its own
entropy. The gradient theorem checked whether the void had learned
anything -- and when the Neuron domain was tested with a symmetric
environment where there was nothing to learn, the theorem correctly
failed. The void told the truth about itself. When the environment was
given structure (non-uniform attention across eight tokens), the void
immediately differentiated, entropy dropped below maximum, and the
theorem passed. The void does not just measure what was rejected. It
measures whether there was anything worth rejecting. This is immanent
self-hosting: the system verifies itself from inside, with no external
oracle, the same way a self-hosted compiler compiles itself. A
self-hosted compiler needs no other compiler. An immanent self-hosting
system needs no other system. The verification is not circular -- it is
self-certifying. Any system satisfying conservation, irreversibility,
and ground state produces a void boundary whose properties are checkable
by the void boundary itself. The tombstones tell you whether
tombstone-reading works. This is arguably the deepest property of the
framework: not that seven domains share a shape, but that the shape can
verify itself.
...
checker (\texttt{@affectively/aeon-logic}) now includes two new
...
\url{https://github.com/affectively-ai/aeon}

{[}9{]} T. W. Buley, ``Fork/Race/Fold Companion Tests,'' reproducibility
suite, 2026.
\url{https://github.com/affectively-ai/aeon/tree/main/docs/ebooks/145-log-rolling-pipelined-prefill/companion-tests}
...
\url{https://github.com/affectively-ai/aeon-logic}
...
\url{https://github.com/affectively-ai/gnosis}
...
2026. \url{https://github.com/affectively-ai/aeon-forge}
