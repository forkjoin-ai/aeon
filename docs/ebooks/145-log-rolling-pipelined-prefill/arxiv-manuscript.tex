...
The three shootoffs --- protocol framing (\S8.5), topological compression
(\S9.2), and laminar pipeline scheduling (\S15.1) --- appear to be
unrelated engineering benchmarks.  They are not.  All three trace the
same Pareto frontier, and that frontier is mechanized.

\textbf{THM-AMERICAN-FRONTIER} (\texttt{AmericanFrontier.lean}) proves that for
any system with intrinsic topology $\beta_1^*$, the map
\[
...
instantiations into a single statement: the topological and
codec-racing frontiers share the same monotone envelope.

Each shootoff is a substrate-specific projection of this frontier:

\begin{itemize}
\item \textbf{Protocol:} HTTP/1.1 ($\beta_1 = 0$) wastes 31\% of wire on
  framing.  Aeon Flow ($\beta_1 = 94$) wastes 1.5\%.  The
  monotone decrease from 31\% to 1.5\% across HTTP/1.1 $\to$ HTTP/2
  $\to$ HTTP/3 $\to$ Aeon Flow is the frontier.
\item \textbf{Pipeline:} At Reynolds number $\text{Re} = 16$ (low
...
\item \textbf{Compression:} Across five corpus types, topology racing
...
the lower bound on waste.  Standard Pareto-analysis tools ---
...
\caption{Figure 3.  The American Frontier: diversity vs waste across three
...
number for a 4-stage pipeline: laminar regime (high diversity, low waste)
to turbulent regime (low diversity, high waste).  \textbf{C.}~Cost of
monoculture: heuristic waste (topology gain \%) across five corpus types
ordered by content heterogeneity, showing that the penalty for
non-diverse strategy scales with intrinsic $\beta_1^*$.  All three
panels are instantiations of THM-AMERICAN-FRONTIER
(\texttt{AmericanFrontier.lean}): waste monotonically non-increasing in
diversity, zero at $\beta_1 = \beta_1^*$, positive below.}
\end{figure}
