...
balanced. Formally, the Lean companion now proves four unbounded lifts:
...
measurable queue observables via \texttt{lintegral}, and a monotone
truncation-to-limit theorem via \texttt{lintegral\_iSup}. This is a
genuine measure-theoretic lifting of the sample-path conservation law
into infinite-support or continuous-support settings. What it still does
not provide is a queue-family-specific stability or ergodic theorem for
...
operationally exact only in the linear full-aggregation regime. Three
...
partition-additive, while winner-selection and early-stop are proved not
to preserve partition additivity or order invariance in general. So the
shared structure is ``fork, independent propagation, recombination to
one output,'' but the recombination mechanics differ: physical path
integrals sum linearly; computational winner/race folds select
nonlinearly.
...
\item
  ...
  \texttt{lintegral}, and monotone truncation-to-limit queueing
  ...
  correspondence, including same-path-family fold ablations and
  fixed-parameter toy-attention behavioral ablations), map/reduce
  readiness diagnostics (boundedness/monotonicity, nonzero-opportunity
  necessity in migration simulation, independent migration-simulator
  rank ordering, and high-readiness counterexample families showing
  non-automatic quantum asymptotics), convergence simulation under the
  three constraints, evidence-table deficits (including T+2 settlement
  ...
  countably supported stochastic, and measure-theoretic queueing lifts,
  plus explicit-assumption theorem schemas (including the stronger
  correspondence-boundary property-negation theorems and global
...
monotone truncation-to-limit queue balance, higher-level queue-limit
...
finite-support arrival cubes; the constructive unbounded lift currently
covers nonnegative measurable observables, countably supported
stochastic laws, and monotone truncation families rather than
state-dependent or ergodic queue-family semantics; and the
...
dynamics. Extending these proofs to richer timing/service distributions,
arbitrary exact multiclass/open networks beyond the current bounded
witnesses, queue-family-specific stability/ergodic theorems for
unbounded open stochastic networks, real-time systems (strict latency
bounds), and full measure-theoretic stochastic semantics with
state-dependent service/routing remains future work.
...
measure-theoretic \texttt{lintegral} conservation, and monotone
truncation-to-limit theorems that lift the sample-path conservation law
into infinite-support or continuous-support settings for nonnegative
measurable observables. Remaining future work is state-dependent
service, vacations, retrials, reneging, arbitrary exact probabilistic
multiclass/open networks beyond the current bounded witnesses, and
queue-family-specific stability/ergodic proofs for unbounded open
stochastic networks.
