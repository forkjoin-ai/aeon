...
The third muse is **stability theory**. A Foster-Lyapunov drift schema treats the safe operating region as a small set and asks whether expected motion points toward it. In this manuscript's geometry, failure is not "the bug happened" but "the drift field turned outward." The topological deficit $\Delta_\beta = \beta_1^* - \beta_1$ is therefore not only an efficiency gap but a failure coordinate: the gap between the manifold the problem needs and the one the implementation can actually sustain.
...
![Figure 2. Artifact-generated regime-map and adversarial-controls figure assembled from the Gnosis regime-sweep and adversarial-control artifacts.](companion-tests/artifacts/ch17-boundary-expansion-figure.svg)
