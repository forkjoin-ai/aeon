...
**THM-PIPELINE-SPEEDUP-FLOOR.** The ceiling above shows speedup approaching $B \times N$. The floor shows pipelining never makes things worse: for any $P \geq 1$, $B \geq 1$, $N \geq 1$, the pipelined time $\lceil P/B \rceil + N - 1$ is $\leq$ the sequential time $P \times N$ (Multiplexing.lean, `pipeline_speedup_floor`). The inequality is strict for $P \geq 2$, $N \geq 2$ (`pipeline_strict_speedup`). The speedup is sandwiched: $1 \leq \text{speedup} \leq B \times N$.
