...
nginx wins on 100 KB files (54,100 vs 21,653 req/s for Aeon Flow at 100 connections). nginx's `sendfile(2)` performs kernel-level zero-copy DMA transfer that bypasses userspace entirely -- the file goes from kernel page cache to socket without touching the application. x-gnosis's flow server reads the file into a `Uint8Array`, encodes it into flow frames, and writes it to the socket. Even with Wallington Rotation chunking (16 KB chunks pipelined as separate DATA frames), the userspace copy cannot be eliminated without native `sendfile` FFI bindings. This is a genuine architectural boundary: the topology advantage (race cache, fold parallelism) is a scheduling optimization that reduces *which* work gets done, while `sendfile` is a data-path optimization that reduces *how* bytes move. Both matter, but they operate at different layers. For large payloads where the cache race always hits (warm cache), the bottleneck shifts entirely to data transfer, and the kernel zero-copy path is architecturally superior.
