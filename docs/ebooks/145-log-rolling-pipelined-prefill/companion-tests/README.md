...
The same formal subtree now also has an explicit Chapter 17 Deceptacon shape layer in `formal/lean/Lean/ForkRaceFoldTheorems/DeceptaconVariants.lean`: legacy `projection` / `search` Deceptacon, explicit `DualVoidDeceptacon` with `voidToggle`, and `TridentDeceptacon` with a live streaming-head branch plus a two-rotation meta-LAMINAR head pipeline.
...
The flagship manuscript now uses that bridge directly through a checked-in witness pair: `artifacts/ch17-wallington-rotation-site.gg` plus `artifacts/ch17-wallington-rotation-site-witness.{json,md}` for the matched zero-deficit case, and `artifacts/ch17-wallington-rotation-positive-deficit-site.gg` plus `artifacts/ch17-wallington-rotation-positive-deficit-site-witness.{json,md}` for the collided positive-deficit case. That keeps the theorem boundary attached to emitted sources instead of hand-described examples.
...
| `ch17-netflix-frontier-figure.test.ts` + `scripts/ch17-netflix-frontier-figure.ts` | §20.2.1 | Netflix Prize as American Frontier: algorithm-family diversity frontier (Panel E) and team-of-teams recursive frontier (Panel F) from published RMSE milestones, with residual gap analysis |
| `ch17-netflix-void-walker.test.ts` | §20.2.2 | Oracle hierarchy: void-walking ensemble vs time-traveled oracle monoculture, god-mode, and per-rating oracle routing on synthetic taste space. Proves the irreducible cost of monoculture |
