...
- The generic adaptive lift still remains honest about its boundary: constructive domination by the supremum or another dominating kernel is proved in `JacksonQueueing.lean`, while the fully generic extraction of the expected-Lyapunov comparison inequalities from arbitrary raw adaptive kernels stays assumption-parameterized in `Axioms.AdaptiveSupremumStabilityAssumptions`.
