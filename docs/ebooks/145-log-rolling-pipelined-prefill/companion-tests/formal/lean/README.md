...
2. `Axioms.AdaptiveSupremumStabilityAssumptions` packages the remaining Lyapunov step as explicit inequalities: if the adaptive expected Lyapunov is pointwise bounded by a ceiling expected Lyapunov, and that ceiling expected Lyapunov satisfies the drift inequality implied by the spectral candidate, then the existing state-dependent stability and terminal-balance schemas apply. At that point positive recurrence and stationary-law existence are derived by the base schema from explicit bridge/drift/small-set witnesses instead of being supplied as free props.
...
The open part is now the generic one you called out: deriving those expected-Lyapunov comparison inequalities and spectral side conditions for arbitrary raw adaptive kernels without supplying them as separate assumptions.
