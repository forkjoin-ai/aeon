  ...
  for (let r = 0; r < maxRounds; r++) {
    ...
    for (let i = 0; i < N; i++) {
      if (!accepts[i]) {
        const bin = Math.min(
          bins - 1,
          Math.floor((candidate[0] / 100) * (bins - 1))
        );
...
function createParty(
  ...
  return {
    ...
    voidCounts: new Array(bins).fill(0),
  };
}
...
describe('Multiparty Negotiation: N-Party Void Walking', () => {
  ...
  it('1. three-party negotiation (buyer, seller, regulator)', () => {
    ...
    console.log(
      `     ${
        result.settled ? `Settled in ${result.rounds} rounds` : 'IMPASSE'
      }`
    );
    if (result.settled) {
      parties.forEach((p, i) =>
        console.log(
          `     ${p.name}: utility=${result.partyUtilities[i].toFixed(1)}`
        )
      );
    }
    console.log(
      `     Coalitions: ${
        result.coalitions.length > 0
          ? result.coalitions.map((c) => c.join('+')).join(', ')
          : 'none'
      }`
    );
  ...
  it('2. coalition detection via void boundary alignment', () => {
    ...
    console.log(
      `     A-B distance: ${l1Distance(
        complementDist(parties[0].voidCounts),
        complementDist(parties[1].voidCounts)
      ).toFixed(3)}`
    );
    console.log(
      `     A-C distance: ${l1Distance(
        complementDist(parties[0].voidCounts),
        complementDist(parties[2].voidCounts)
      ).toFixed(3)}`
    );
    console.log(
      `     Coalitions: ${
        coalitions.map((c) => c.join('+')).join(', ') || 'none detected'
      }`
    );

    // A and B should be closer to each other than to C
    const abDist = l1Distance(
      complementDist(parties[0].voidCounts),
      complementDist(parties[1].voidCounts)
    );
    const acDist = l1Distance(
      complementDist(parties[0].voidCounts),
      complementDist(parties[2].voidCounts)
    );
  ...
  it('3. veto player blocks settlement', () => {
    ...
    console.log(
      `     ${
        result.settled
          ? `Settled in ${result.rounds}`
          : 'BLOCKED (veto player BATNA too high)'
      }`
    );
  ...
  it('4. N-party public goods: free-rider void accumulates', () => {
    ...
    console.log(
      `     Contributions: ${totalContributions}  Free-rides: ${totalFreeRides}`
    );
    console.log(
      `     Contribution rate: ${(
        (totalContributions / (totalContributions + totalFreeRides)) *
        100
      ).toFixed(1)}%`
    );
  ...
  it('5. UN Security Council model: veto + majority', () => {
    ...
    console.log(
      `     ${
        settled
          ? `Resolution passed in ${rounds} rounds`
          : `No resolution (${rounds} rounds)`
      }`
    );
  ...
  it('6. unanimous vs majority: settlement rate comparison', () => {
    ...
    console.log(
      `     Unanimous: ${unanimousSettled}/${trials} settled (${(
        (unanimousSettled / trials) *
        100
      ).toFixed(0)}%)`
    );
    console.log(
      `     Majority:  ${majoritySettled}/${trials} settled (${(
        (majoritySettled / trials) *
        100
      ).toFixed(0)}%)`
    );
  ...
  it('7. semiotic deficit scales with party count', () => {
    ...
    for (const r of results) {
      console.log(
        `     ${String(r.n).padStart(6)}  ${String(r.deficit).padStart(
          7
        )}  ${String(r.rounds).padStart(6)}  ${
          r.settled ? 'settled' : 'IMPASSE'
        }`
      );
    }
