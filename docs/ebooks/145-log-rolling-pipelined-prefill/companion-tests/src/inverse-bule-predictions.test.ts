...
const SH: PayoffFn = (my, opp) => ([[4, 4], [0, 2], [2, 0], [2, 2]] as [number, number][])[my * 2 + opp];

function runGame(
  N: number, T: number, payoff: PayoffFn,
  oppFn: (rng: () => number) => number, rng: () => number,
): { voidCounts: number[]; totalPayoff: number; inverseBuleAtHalf: number; inverseBuleAtEnd: number; settled: boolean } {
  ...
  for (let r = 0; r < T; r++) {
    ...
    if (r === Math.floor(T / 2)) inverseBuleAtHalf = computeInverseBule(void_, r + 1);
  }
  return {
    voidCounts: void_,
    totalPayoff,
...
describe('Inverse Bule as Predictive Metric', () => {
  ...
  it('2. B⁻¹ at round 10 predicts total rounds needed', () => {
    ...
    console.log(`  2. B⁻¹ at round 10 vs total payoff: Spearman ρ = ${rho.toFixed(3)}`);
    expect(isFinite(rho)).toBe(true);
  });
  ...
  it('3. B⁻¹ trajectory distinguishes strategies', () => {
    ...
    for (const p of profiles.sort((a, b) => b.bule - a.bule)) {
      console.log(`     ${p.name.padEnd(20)} B⁻¹ = ${(p.bule * 1000).toFixed(3)} mB⁻¹`);
    }
  ...
  it('4. B⁻¹ drops before regime change', () => {
    ...
    for (let r = 0; r < T; r++) {
      ...
      const [p, q] = PD(choice, opp);
      if (p < q) void_[choice]++;
      if ((r + 1) % 20 === 0) buleTrajectory.push(computeInverseBule(void_, r + 1));
    }
    ...
    const beforeMean = beforeSwitch.reduce((a, b) => a + b, 0) / beforeSwitch.length;
    const afterMean = afterSwitch.reduce((a, b) => a + b, 0) / afterSwitch.length;

    console.log(`  4. B⁻¹ around regime change: before=${(beforeMean * 1000).toFixed(3)} after=${(afterMean * 1000).toFixed(3)} mB⁻¹`);
  ...
  it('5. B⁻¹ is non-negative under stationary costs (supermartingale)', () => {
    ...
    for (let r = 0; r < 500; r++) {
      ...
      let choice = 2;
      let cum = 0;
      for (let i = 0; i < 3; i++) { cum += dist[i]; if (rv < cum) { choice = i; break; } }
    ...
    console.log(`  5. B⁻¹ non-negative: min=${(Math.min(...buleValues) * 1000).toFixed(3)} max=${(Math.max(...buleValues) * 1000).toFixed(3)} mB⁻¹`);
  });
  ...
  it('6. B⁻¹ is scale-invariant under payoff rescaling', () => {
    ...
    console.log(`  6. Scale invariance: original B⁻¹=${(r1.inverseBuleAtEnd * 1000).toFixed(3)} scaled(10x) B⁻¹=${(r2.inverseBuleAtEnd * 1000).toFixed(3)} mB⁻¹`);
  ...
  it('7. B⁻¹ is invariant under choice relabeling', () => {
    ...
    const r2 = runGame(2, 200, relabeledPD, (rng) => rng() < 0.5 ? 1 : 0, rng2);

    // B⁻¹ should be identical (relabeling doesn't change information content)
    console.log(`  7. Relabeling invariance: original B⁻¹=${(r1.inverseBuleAtEnd * 1000).toFixed(3)} relabeled B⁻¹=${(r2.inverseBuleAtEnd * 1000).toFixed(3)} mB⁻¹`);
    // Should be very close (same game, different labels)
    expect(Math.abs(r1.inverseBuleAtEnd - r2.inverseBuleAtEnd)).toBeLessThan(0.01);
  });
  ...
  it('8. B⁻¹ distinguishes NT from AUT perceptual profiles', () => {
    ...
    console.log(`  8. NT B⁻¹=${(ntBule * 1000).toFixed(3)} AUT B⁻¹=${(autBule * 1000).toFixed(3)} mB⁻¹`);
    console.log(`     NT filters faster (higher B⁻¹). AUT reads more (lower B⁻¹, higher H).`);
  ...
  it('9. Cross-game transfer: B⁻¹ in PD correlates with performance in Stag Hunt', () => {
    ...
    for (let s = 0; s < trials; s++) {
      ...
      const pdResult = runGame(2, 100, PD, (rng) => rng() < 0.5 ? 0 : 1, rng1);
      const shResult = runGame(2, 100, SH, (rng) => rng() < 0.5 ? 0 : 1, rng2);
      pdBules.push(pdResult.inverseBuleAtEnd);
      shPayoffs.push(shResult.totalPayoff);
    }

    const rho = spearmanRank(pdBules, shPayoffs);
    console.log(`  9. Cross-game transfer: PD B⁻¹ vs SH payoff: Spearman ρ = ${rho.toFixed(3)}`);
    expect(isFinite(rho)).toBe(true);
  });
  ...
  it('10. Effect size: Cohen d between void-walker and random strategy', () => {
    ...
    console.log(`      ${Math.abs(cohensD) > 0.8 ? 'LARGE' : Math.abs(cohensD) > 0.5 ? 'MEDIUM' : 'SMALL'} effect size`);
