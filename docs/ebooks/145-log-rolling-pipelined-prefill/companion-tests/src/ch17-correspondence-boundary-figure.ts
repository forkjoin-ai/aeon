  ...
  return {
    ...
    toyAttention: {
      ...
      mseCi95: {
        ...
        'winner-take-all': toyAttention.strategies['winner-take-all'].meanSquaredErrorCi95,
        'early-stop': toyAttention.strategies['early-stop'].meanSquaredErrorCi95,
      },
      exactFraction: {
    ...
    gnosisTraining: {
      ...
      evalMseCi95: {
        ...
        'early-stop': gnosisTraining.strategies['early-stop'].evalMeanSquaredErrorCi95,
      },
      exactFraction: {
    ...
    gnosisMiniMoe: {
      evalMse: {
        ...
        'early-stop': gnosisMiniMoe.strategies['early-stop'].meanEvalMeanSquaredError,
      },
      evalMseCi95: {
        ...
        'early-stop': gnosisMiniMoe.strategies['early-stop'].evalMeanSquaredErrorCi95,
      },
      exactFraction: {
        linear: gnosisMiniMoe.strategies.linear.meanExactWithinToleranceFraction,
        'winner-take-all':
          gnosisMiniMoe.strategies['winner-take-all'].meanExactWithinToleranceFraction,
        'early-stop': gnosisMiniMoe.strategies['early-stop'].meanExactWithinToleranceFraction,
      },
      dualActiveAbsError: {
        linear: gnosisMiniMoe.strategies.linear.meanDualActiveRegionMeanAbsoluteError,
        'winner-take-all':
          gnosisMiniMoe.strategies['winner-take-all'].meanDualActiveRegionMeanAbsoluteError,
        'early-stop': gnosisMiniMoe.strategies['early-stop'].meanDualActiveRegionMeanAbsoluteError,
      },
    },
  };
}
...
export function renderCh17CorrespondenceBoundaryFigureMarkdown(
  ...
  lines.push(`- Gnosis mini-MoE source: \`${report.sources.gnosisMiniMoeLabel}\``);
  lines.push('');
  ...
  lines.push('');
  lines.push('| Strategy | Toy attention MSE | Toy 95% CI | Gnosis cancellation MSE | Gnosis cancellation 95% CI | Gnosis mini-MoE MSE | Gnosis mini-MoE 95% CI |');
  lines.push('|---|---:|---:|---:|---:|---:|---:|');
  ...
  for (const strategy of ['linear', 'winner-take-all', 'early-stop'] as StrategyName[]) {
    ...
    lines.push(
      `| \`${strategy}\` | ${formatNumber(report.toyAttention.mse[strategy])} | [${formatNumber(toyCi.low)}, ${formatNumber(toyCi.high)}] | ${formatNumber(report.gnosisTraining.evalMse[strategy])} | [${formatNumber(trainingCi.low)}, ${formatNumber(trainingCi.high)}] | ${formatNumber(report.gnosisMiniMoe.evalMse[strategy])} | [${formatNumber(miniMoeCi.low)}, ${formatNumber(miniMoeCi.high)}] |`,
    );
  }
...
function renderMetricPanel(svg: string[], config: MetricPanelConfig): void {
  ...
  const maxValue = Math.max(
    ...strategies.map((strategy) => Math.max(config.values[strategy], config.intervals[strategy].high)),
  );
  ...
  svg.push(
    `<text x="${config.x + 24}" y="${config.y + 36}" font-family="Georgia, serif" font-size="22" fill="#111827">${escapeXml(
      config.title,
    )}</text>`,
  );
  svg.push(
    `<text x="${config.x + 24}" y="${config.y + 60}" font-family="Georgia, serif" font-size="13" fill="#57534e">${escapeXml(
      config.subtitle,
    )}</text>`,
  );
  svg.push(
    `<text x="${config.x + 24}" y="${config.y + 86}" font-family="ui-monospace, SFMono-Regular, monospace" font-size="12" fill="#374151">${escapeXml(
      config.axisLabel,
    )}</text>`,
  );
  svg.push(
    `<line x1="${config.x + 44}" y1="${chartBottom}" x2="${config.x + config.width - 30}" y2="${chartBottom}" stroke="#9ca3af" stroke-width="1"/>`,
  );
  ...
  strategies.forEach((strategy, index) => {
    ...
    const intervalLowY = chartBottom - barHeight(interval.low, maxValue, chartHeight);
    const intervalHighY = chartBottom - barHeight(interval.high, maxValue, chartHeight);
    ...
    svg.push(
      `<line x1="${centerX - 10}" y1="${intervalHighY}" x2="${centerX + 10}" y2="${intervalHighY}" stroke="#111827" stroke-width="2"/>`,
    );
    svg.push(
    ...
    svg.push(
      `<text x="${centerX}" y="${chartBottom + 24}" text-anchor="middle" font-family="ui-monospace, SFMono-Regular, monospace" font-size="12" fill="#111827">${escapeXml(
        strategy,
      )}</text>`,
    );
    svg.push(
      `<text x="${centerX}" y="${chartBottom + 42}" text-anchor="middle" font-family="ui-monospace, SFMono-Regular, monospace" font-size="11" fill="#6b7280">${escapeXml(
        `${config.secondaryLabel} ${formatNumber(config.secondaryValues[strategy])}`,
      )}</text>`,
    );
  });
}
...
export function renderCh17CorrespondenceBoundaryFigureSvg(
  ...
  svg.push('<desc id="desc">Invariant matrix, toy-attention error chart with bootstrap intervals, seeded Gnosis cancellation benchmark with seed intervals, and seeded Gnosis mini-MoE routing benchmark with seed intervals.</desc>');
  svg.push('<defs>');
  ...
  svg.push('<text x="48" y="56" font-family="Georgia, serif" font-size="28" fill="#111827">Chapter 17 Correspondence Boundary</text>');
  svg.push('<text x="48" y="84" font-family="Georgia, serif" font-size="14" fill="#4b5563">Invariant matrix plus three interval-backed behavioral benchmarks</text>');

  svg.push('<rect x="40" y="120" width="560" height="350" rx="20" fill="#fffaf0" stroke="#d6d3d1"/>');
