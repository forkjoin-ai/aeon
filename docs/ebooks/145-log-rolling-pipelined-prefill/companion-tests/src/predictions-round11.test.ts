...
interface Branch { survives: boolean; output: number[]; }
...
describe('Prediction 172: Universal Collapse Cost Floor', () => {
  it('collapse cost = liveBranches - 1 (achievable)', () => {
    ...
    const cost = ventedCount(branches, collapsed) + repairDebt(branches, collapsed);
...
describe('Prediction 174: Exact Venting Cost', () => {
  it('venting = liveBranches - 1', () => {
    for (const n of [2, 5, 10, 20]) {
      const branches = Array.from({ length: n }, () => ({ survives: true, output: [0] }));
      const collapsed = collapseWitness(branches);
...
describe('Prediction 176: Path Conservation', () => {
  it('live = witness_live + witness_vented', () => {
    for (const n of [3, 7, 15]) {
      const branches = Array.from({ length: n }, () => ({ survives: true, output: [0] }));
      const collapsed = collapseWitness(branches);
      expect(liveBranchCount(branches))
        .toBe(liveBranchCount(collapsed) + ventedCount(branches, collapsed));
    }
  });
  it('remainder kills all', () => {
    const branches = Array.from({ length: 5 }, () => ({ survives: true, output: [0] }));
    const remainder = branches.map(b => ({ survives: false, output: b.output }));
...
describe('Round 11 Master', () => {
  it('all five verified', () => {
    const b = Array.from({ length: 6 }, () => ({ survives: true, output: [0] }));
