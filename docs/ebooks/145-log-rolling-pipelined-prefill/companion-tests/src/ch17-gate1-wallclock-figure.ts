...
export function buildCh17Gate1WallclockFigureReport(
  ...
  const cells = report.cells.map<Gate1FigureCell>((cell) => ({
    ...
    networkLabel: `RTT ${trimFixed(cell.network.rttMs, 1)} ms • loss ${formatLossPercent(cell.network.lossRate)}`,
  ...
  const minSpeedupCiLow = Math.min(...cells.map((cell) => cell.speedupMedianCi.low));
...
export function renderCh17Gate1WallclockFigureMarkdown(
  ...
  lines.push(`- Distinct endpoint hosts: \`${report.distinctEndpointHostCount}\``);
  lines.push(`- Primary cells passed: \`${report.primaryPassed}/${report.primaryTotal}\``);
  lines.push(
    `- Speedup median range: \`${formatRatio(report.speedupMedianRange.low)}\` to \`${formatRatio(report.speedupMedianRange.high)}\``,
  );
  lines.push(`- Minimum speedup CI low: \`${formatRatio(report.minSpeedupCiLow)}\``);
  lines.push(
    `- Minimum latency-improvement CI low: \`${formatMs(report.minImprovementCiLowMs)} ms\``,
  );
  lines.push('');
  lines.push('## Cells');
  lines.push('');
  ...
  for (const cell of report.cells) {
    lines.push(
      `| ${cell.cellId} | ${cell.workloadLabel} | ${cell.networkLabel} | ${cell.primary ? 'yes' : 'no'} | ${formatMs(cell.sequentialP50Ms)} | ${formatMs(cell.chunkedP50Ms)} | ${formatRatio(cell.speedupMedian)} (${formatRatio(cell.speedupMedianCi.low)} to ${formatRatio(cell.speedupMedianCi.high)}) | ${formatMs(cell.improvementMedianMs)} (${formatMs(cell.improvementMedianMsCi.low)} to ${formatMs(cell.improvementMedianMsCi.high)}) |`,
    );
  }

  lines.push('');
  lines.push('Interpretation: the figure pairs a log-scale p50 latency dumbbell with speedup confidence intervals so the deployment-level separation is visible without collapsing the result into a single summary number.');
...
export function renderCh17Gate1WallclockFigureSvg(
  ...
  const speedupMax =
    Math.ceil((Math.max(...rows.map((cell) => cell.speedupMedianCi.high)) + 1) / 2) * 2;
  ...
  svg.push(
    `<text x="48" y="88" font-family="Georgia, serif" font-size="15" fill="#4b5563">Source ${escapeXml(report.sourceLabel)} • ${report.distinctEndpointHostCount} distinct hosts • ${report.primaryPassed}/${report.primaryTotal} primary cells pass</text>`,
  );
  svg.push(
  ...
  for (const tick of latencyTicks) {
    ...
    svg.push(
      `<text x="${x}" y="${axisY + 24}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="12" fill="#6b7280">${formatMs(tick)}</text>`,
    );
  }
  svg.push(
    `<text x="${latencyPanelX + latencyPanelWidth / 2}" y="${axisLabelY}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="12" fill="#6b7280">p50 latency (ms, log scale)</text>`,
  );

  for (const tick of speedupTicks) {
    const x = speedupScale(tick, speedupMin, speedupMax, speedupPanelX, speedupPanelWidth);
    svg.push(
      `<line x1="${x}" y1="208" x2="${x}" y2="${axisY}" stroke="#e5e7eb" stroke-width="1"/>`,
    );
    svg.push(
      `<text x="${x}" y="${axisY + 24}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="12" fill="#6b7280">${tick}</text>`,
    );
  }
  svg.push(
    `<text x="${speedupPanelX + speedupPanelWidth / 2}" y="${axisLabelY}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="12" fill="#6b7280">median speedup (x)</text>`,
  );
  svg.push(
  ...
  svg.push(
    `<text x="110" y="${legendY + 5}" font-family="system-ui, sans-serif" font-size="12" fill="#4b5563">chunked p50</text>`,
  );
  svg.push(
    `<circle cx="188" cy="${legendY}" r="6" fill="${latencyColor('sequential')}"/>`,
  );
  svg.push(
    `<text x="202" y="${legendY + 5}" font-family="system-ui, sans-serif" font-size="12" fill="#4b5563">sequential p50</text>`,
  );
  svg.push(`<line x1="318" y1="${legendY - 6}" x2="354" y2="${legendY - 6}" stroke="#4b5563" stroke-width="3"/>`);
  svg.push(
    `<circle cx="336" cy="${legendY - 6}" r="5" fill="#0f766e"/>`,
  );
  svg.push(
    `<text x="366" y="${legendY - 1}" font-family="system-ui, sans-serif" font-size="12" fill="#4b5563">speedup median with CI</text>`,
  );
  ...
  rows.forEach((cell, index) => {
    ...
    const chunkCiHigh = latencyScale(cell.chunkedP50Ci.high, latencyPanelX, latencyPanelWidth);
    const seqCiLow = latencyScale(cell.sequentialP50Ci.low, latencyPanelX, latencyPanelWidth);
    const seqCiHigh = latencyScale(cell.sequentialP50Ci.high, latencyPanelX, latencyPanelWidth);
    const chunkX = latencyScale(cell.chunkedP50Ms, latencyPanelX, latencyPanelWidth);
    const seqX = latencyScale(cell.sequentialP50Ms, latencyPanelX, latencyPanelWidth);

    svg.push(
      `<line x1="${chunkCiLow}" y1="${y + 8}" x2="${chunkCiHigh}" y2="${y + 8}" stroke="${latencyColor('chunked')}" stroke-width="3" opacity="0.35"/>`,
    );
    svg.push(
      `<line x1="${seqCiLow}" y1="${y - 10}" x2="${seqCiHigh}" y2="${y - 10}" stroke="${latencyColor('sequential')}" stroke-width="3" opacity="0.35"/>`,
    );
    svg.push(
      `<line x1="${chunkX}" y1="${y - 1}" x2="${seqX}" y2="${y - 1}" stroke="#94a3b8" stroke-width="2"/>`,
    );
    svg.push(
      `<circle cx="${chunkX}" cy="${y - 1}" r="5.5" fill="${latencyColor('chunked')}" stroke="#ffffff" stroke-width="1.5"/>`,
    );
    svg.push(
      `<circle cx="${seqX}" cy="${y - 1}" r="5.5" fill="${latencyColor('sequential')}" stroke="#ffffff" stroke-width="1.5"/>`,
    );
    ...
    svg.push(
      `<line x1="${speedupCiLowX}" y1="${y - 1}" x2="${speedupCiHighX}" y2="${y - 1}" stroke="${dotFill}" stroke-width="3" opacity="${dotOpacity}"/>`,
    );
    svg.push(
      `<line x1="${speedupCiLowX}" y1="${y - 8}" x2="${speedupCiLowX}" y2="${y + 6}" stroke="${dotFill}" stroke-width="1.5" opacity="${dotOpacity}"/>`,
    );
    svg.push(
      `<line x1="${speedupCiHighX}" y1="${y - 8}" x2="${speedupCiHighX}" y2="${y + 6}" stroke="${dotFill}" stroke-width="1.5" opacity="${dotOpacity}"/>`,
    );
    svg.push(
      `<circle cx="${speedupMedianX}" cy="${y - 1}" r="5.5" fill="${dotFill}" opacity="${dotOpacity}"/>`,
    );
    svg.push(
      `<text x="${improvementColumnX}" y="${y + 4}" font-family="system-ui, sans-serif" font-size="12" fill="#374151">+${formatMs(cell.improvementMedianMs)} ms</text>`,
    );
  });
