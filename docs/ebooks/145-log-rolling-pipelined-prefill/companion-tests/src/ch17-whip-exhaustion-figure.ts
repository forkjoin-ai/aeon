...
export function buildCh17WhipExhaustionFigureReport(
  config: WhipExhaustionLayerConfig = { heads: 16, ffnExpansion: 4, moeExperts: 0, moeTopK: 0 },
  ...
  const outermost = Math.min(...axes.filter((a) => a.beta1 > 0).map((a) => a.beta1));
...
export function renderCh17WhipExhaustionFigureSvg(
  ...
  // --- Title ---
  svgLabel(svg, W / 2, 38, 'Whip Exhaustion: Transformer Fork Dimension Completeness', {
    size: 22,
    weight: 700,
  });
  svgLabel(svg, W / 2, 62, `N=${report.config.heads} heads, ${report.config.moeExperts > 0 ? `E=${report.config.moeExperts} MoE experts` : `${report.config.ffnExpansion}x FFN`} — Total beta_1 = ${report.totalBeta1}, ${report.whipCount} whip snaps per layer`, {
    size: 13,
  ...
  svgRect(svg, centerX - nodeW / 2, curY, nodeW, nodeH, '#eef2ff', COLORS.residual);
  svgLabel(svg, centerX, curY + 17, 'Residual Fork 1', { size: 12, weight: 600, color: COLORS.residual });
  svgLabel(svg, centerX, curY + 32, 'beta_1 += 1', { size: 10, color: COLORS.textLight, italic: true });
  svgCurve(svg, `M ${centerX} ${inputY + 16} L ${centerX} ${curY}`, COLORS.flow, { marker: true });
  ...
  svgLabel(svg, identX, curY + 18, 'Identity', { size: 11, color: COLORS.textLight });
  svgCurve(svg, `M ${centerX - nodeW / 4} ${resFork1Y + nodeH} Q ${identX} ${resFork1Y + nodeH + 6} ${identX} ${curY}`, COLORS.flow, { dashed: true });
  const identTopY = curY;

  // Attention sublayer (right)
  svgRect(svg, attnX - 60, curY, 120, 30, '#f0fdf4', COLORS.attnHead);
  svgLabel(svg, attnX, curY + 18, 'Attention', { size: 11, weight: 600, color: COLORS.attnHead });
  svgCurve(svg, `M ${centerX + nodeW / 4} ${resFork1Y + nodeH} Q ${attnX} ${resFork1Y + nodeH + 6} ${attnX} ${curY}`, COLORS.attnHead, { marker: true });
  ...
  svgLabel(svg, attnX, curY + 17, `Head Fork (N=${report.config.heads})`, { size: 12, weight: 600, color: COLORS.attnHead });
  svgLabel(svg, attnX, curY + 32, `beta_1 += ${report.config.heads - 1}`, { size: 10, color: COLORS.textLight, italic: true });
  svgCurve(svg, `M ${attnX} ${curY - 10} L ${attnX} ${curY}`, COLORS.attnHead, { marker: true });
  ...
  for (let i = 0; i < headCount; i++) {
    ...
    if (i < 3 || i === headCount - 1) {
      svgLabel(svg, hx, headDotY + 14, i < 3 ? `h${i + 1}` : `h${report.config.heads}`, { size: 8, color: COLORS.textLight });
    }
    if (i === 3 && headCount > 4) {
      svgLabel(svg, hx, headDotY + 14, '...', { size: 10, color: COLORS.textLight });
    }
  }
  ...
  svgLabel(svg, attnX, curY + 14, 'Concat + W_O', { size: 11, weight: 600, color: COLORS.snap });
  svgLabel(svg, attnX, curY + 28, `WHIP 1: discharges ${report.config.heads - 1} cycles`, { size: 9, weight: 700, color: COLORS.snap });
  // Fan in lines from heads to fold
  for (let i = 0; i < headCount; i++) {
    const hx = attnX - headSpread / 2 + (headSpread / (headCount - 1)) * i;
    svgCurve(svg, `M ${hx} ${headDotY + 5} L ${attnX} ${curY}`, COLORS.snap, { width: 1.2 });
  }
  ...
  svgLabel(svg, centerX, curY + 14, 'Additive Merge', { size: 11, weight: 600, color: COLORS.snap });
  svgLabel(svg, centerX, curY + 28, 'WHIP 2: discharges 1 cycle', { size: 9, weight: 700, color: COLORS.snap });
  // Join identity + attention output
  svgCurve(svg, `M ${identX} ${identTopY + 30} L ${identX} ${curY + 18} L ${centerX - nodeW / 2} ${curY + 18}`, COLORS.flow, { dashed: true });
  svgCurve(svg, `M ${attnX} ${headFoldY + 36} L ${attnX} ${curY + 18} L ${centerX + nodeW / 2} ${curY + 18}`, COLORS.snap);
  ...
  svgRect(svg, centerX - nodeW / 2, curY, nodeW, nodeH, '#eef2ff', COLORS.residual);
  svgLabel(svg, centerX, curY + 17, 'Residual Fork 2', { size: 12, weight: 600, color: COLORS.residual });
  svgLabel(svg, centerX, curY + 32, 'beta_1 += 1', { size: 10, color: COLORS.textLight, italic: true });
  svgCurve(svg, `M ${centerX} ${resFold1Y + 36} L ${centerX} ${curY}`, COLORS.flow, { marker: true });
  ...
  svgLabel(svg, ident2X, curY + 18, 'Identity', { size: 11, color: COLORS.textLight });
  svgCurve(svg, `M ${centerX - nodeW / 4} ${resFork2Y + nodeH} Q ${ident2X} ${resFork2Y + nodeH + 6} ${ident2X} ${curY}`, COLORS.flow, { dashed: true });
  const ident2TopY = curY;

  svgRect(svg, ffnX - 60, curY, 120, 30, useMoe ? '#fef2f2' : '#fffbeb', ffnAxis.color);
  svgLabel(svg, ffnX, curY + 18, useMoe ? 'MoE' : 'FFN', { size: 11, weight: 600, color: ffnAxis.color });
  svgCurve(svg, `M ${centerX + nodeW / 4} ${resFork2Y + nodeH} Q ${ffnX} ${resFork2Y + nodeH + 6} ${ffnX} ${curY}`, ffnAxis.color, { marker: true });

  curY += 40;

  // FFN/MoE Fork
  svgRect(svg, ffnX - 80, curY, 160, nodeH, useMoe ? '#fee2e2' : '#fef3c7', ffnAxis.color);
  svgLabel(svg, ffnX, curY + 17, ffnAxis.label, { size: 11, weight: 600, color: ffnAxis.color });
  svgLabel(svg, ffnX, curY + 32, `beta_1 += ${ffnAxis.beta1}`, { size: 10, color: COLORS.textLight, italic: true });
  svgCurve(svg, `M ${ffnX} ${curY - 10} L ${ffnX} ${curY}`, ffnAxis.color, { marker: true });

  curY += nodeH + 20;

  // FFN/MoE Fold — WHIP 3
  svgRect(svg, ffnX - 80, curY, 160, 36, '#fef2f2', COLORS.snap);
  svgLabel(svg, ffnX, curY + 14, ffnAxis.foldLabel, { size: 10, weight: 600, color: COLORS.snap });
  svgLabel(svg, ffnX, curY + 28, `WHIP 3: discharges ${ffnAxis.beta1} cycles`, { size: 9, weight: 700, color: COLORS.snap });
  const ffnFoldY = curY;
  ...
  svgRect(svg, centerX - nodeW / 2, curY, nodeW, 36, '#fef2f2', COLORS.snap);
  svgLabel(svg, centerX, curY + 14, 'Additive Merge', { size: 11, weight: 600, color: COLORS.snap });
  svgLabel(svg, centerX, curY + 28, 'WHIP 4: discharges 1 cycle', { size: 9, weight: 700, color: COLORS.snap });
  svgCurve(svg, `M ${ident2X} ${ident2TopY + 30} L ${ident2X} ${curY + 18} L ${centerX - nodeW / 2} ${curY + 18}`, COLORS.flow, { dashed: true });
  svgCurve(svg, `M ${ffnX} ${ffnFoldY + 36} L ${ffnX} ${curY + 18} L ${centerX + nodeW / 2} ${curY + 18}`, COLORS.snap);
  ...
  svgCurve(svg, `M ${centerX} ${resFold2Y + 36} L ${centerX} ${curY - 16}`, COLORS.flow, { marker: true });
  svgLabel(svg, centerX, curY + 30, 'beta_1 = 0', { size: 11, weight: 700, color: COLORS.attnHead });
  ...
  svgLabel(svg, chartX + chartW / 2, chartY, 'Energy Taper: Cycles Discharged per Whip', {
    size: 15,
    weight: 700,
  });
  ...
  svg.push(
    `<rect x="${chartX + 20}" y="${chartY + 20}" width="${chartW - 40}" height="${chartH - 65}" rx="8" fill="url(#taper-grad)"/>`,
  );
  ...
  svgLabel(svg, chartX + chartW / 2, exclY + 22, 'Excluded (Non-Fork) Dimensions', {
    size: 15,
    weight: 700,
  });

  const excluded = [
    { label: 'Layer stack (L layers)', reason: 'Causal dependency', beta1: '0' },
    { label: 'Sequence positions (T tokens)', reason: 'Entangled via QK^T', beta1: '0' },
    { label: 'Batch dimension (B samples)', reason: 'No cross-sample fold', beta1: '0' },
    { label: 'KV cache positions', reason: 'Sequential accumulation', beta1: '0' },
  ];
  ...
  for (const item of excluded) {
    ...
    svgLabel(svg, rowX + 8, exY + 12, 'X', { size: 10, weight: 700, color: COLORS.snap });
    svgLabel(svg, rowX + 26, exY + 12, item.label, { anchor: 'start', size: 12, weight: 600 });
    svgLabel(svg, rowX + 26, exY + 30, item.reason, { anchor: 'start', size: 11, color: COLORS.textLight, italic: true });
    svgBadge(svg, chartX + chartW - 50, exY + 16, `beta_1 = ${item.beta1}`, '#f1f5f9', '#e2e8f0', COLORS.textLight);
    exY += 54;
  }
  ...
  svgLabel(svg, W / 2, summaryY + 22, `Conservation: ${report.totalBeta1} cycles created = ${report.totalBeta1} cycles discharged = beta_1(layer). After 4 folds, beta_1 = 0.`, {
    size: 13,
    weight: 600,
    color: COLORS.attnHead,
  });
  svgLabel(svg, W / 2, summaryY + 44, `Energy taper ratio: ${report.taperRatio.toFixed(0)}:1 (inner whip discharges ${report.taperRatio.toFixed(0)}x more cycles than outer). No potential escapes the layer boundary.`, {
    size: 12,
    color: COLORS.textLight,
    italic: true,
  });
...
export function renderCh17WhipExhaustionFigureMarkdown(
  ...
  const lines: string[] = [
    ...
    '',
    `**Configuration:** N=${report.config.heads} heads, ${report.config.moeExperts > 0 ? `E=${report.config.moeExperts} MoE experts (top-${report.config.moeTopK})` : `${report.config.ffnExpansion}x FFN expansion`}`,
    '',
    '## Fork Axes',
    '',
  ...
  const sorted = [...report.axes].filter((a) => a.beta1 > 0).sort((a, b) => b.beta1 - a.beta1);
  ...
  lines.push(
    '',
    `The innermost fold (attention heads) discharges ${report.taperRatio.toFixed(0)}x more cycles than each outermost fold (residual). This is the computational taper: energy density concentrates inward, matching the physical whip where the tip carries the most momentum.`,
    '',
  );
