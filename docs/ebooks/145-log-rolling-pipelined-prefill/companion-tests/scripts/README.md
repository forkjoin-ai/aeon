...
- `run-lean-theorems.ts`: runs `@a0n/aeon-logic`'s `runLeanSandbox` against the in-tree `formal/lean` package, requiring a real `lake build` success and surfacing the sandbox report; accepts optional comma-separated `AEON_LEAN_BUILD_TARGETS` to focus the Lean build on specific modules without changing the default full entrypoint target. Compiler-side emitted-kernel theorems in `open-source/gnosis/GnosisProofs.lean` are rerun separately through the wrapper script `bun run test:formal:gnosis`.
- `run-lean-theorems.sh`: shell wrapper that forwards to `run-lean-theorems.ts` for environments that expect the historical entrypoint; like the TypeScript entrypoint, it covers `formal/lean` only and does not replace the separate `test:formal:gnosis` / `GnosisProofs.lean` build path.
- `validate-formal-artifacts.ts`: preflights all formal artifacts via `@a0n/aeon-logic` (`.tla/.cfg` parse + round-trip checks plus Lean project inspection through `runLeanSandbox` with `build: false`).
...
- `sleep-debt-bounded-witness.ts`: bounded sleep-debt witness runner that writes JSON/Markdown artifacts for full recovery, partial recovery, and chronic truncation scenarios.
- `sleep-debt-schedule-threshold-witness.ts`: coarse schedule-threshold witness runner that writes JSON/Markdown artifacts for subcritical, critical, and supercritical repeated-cycle schedules.
- `sleep-debt-weighted-threshold-witness.ts`: weighted schedule-threshold bridge runner that writes JSON/Markdown artifacts for the integerized `20.2 h` critical boundary and the resulting subcritical/critical/supercritical repeated-cycle cases.
...
- `ch17-gate1-wallclock-figure.ts`: artifact assembler that reads the six-host Gate 1 wall-clock report and writes manuscript-ready JSON/Markdown/SVG outputs for the deployment-latency and speedup surface.
- `ch17-gate2-protocol-corpus-figure.ts`: artifact assembler that reads the Gate 2 protocol-corpus report and writes manuscript-ready JSON/Markdown/SVG outputs for the framing and latency-gain surface.
- `ch17-gate3-compression-corpus-figure.ts`: artifact assembler that reads the Gate 3 compression-corpus report and writes manuscript-ready JSON/Markdown/SVG outputs for the dual compression-gain surface.
- `ch17-gate4-rqr-holdout-figure.ts`: artifact assembler that reads the Gate 4 holdout report and writes manuscript-ready JSON/Markdown/SVG outputs for the decile-calibration and screening-criteria surface.
- `ch17-gate5-bio-effect-size-figure.ts`: artifact assembler that reads the Gate 5 biological pair report and writes manuscript-ready JSON/Markdown/SVG outputs for the log-scale forest-plot summary surface.
- `ch17-inverted-scaling-reynolds-figure.ts`: analytic figure assembler that evaluates the Chapter 17 formulas and writes manuscript-ready JSON/Markdown/SVG outputs for the inverted-scaling and Reynolds-regime surface.
- `ch17-american-frontier-figure.ts`: artifact assembler that writes the American Frontier curve family as manuscript-ready JSON/Markdown/SVG outputs, including the explicit Aeon/UDP vs HTTP/TCP mixed-race witness for inverse-Bule-driven transport tuning.
...
- `ch17-cosmic-explainer-figure.ts`: artifact assembler that reads the manuscript-authored cosmic `aeon-viz` scene and writes synchronized JSON/Markdown/SVG outputs for the golden-spiral time/scale explainer.
- `ch17-dimension-ladder-figure.ts`: artifact assembler that reads the manuscript-authored dimensional `aeon-viz` scene and writes synchronized JSON/Markdown/SVG outputs for the `2D..55D` ladder plus projected `54D` particle explainer.
...
- `ch17-moa-whip-curvature-figure.ts`: supplemental topology artifact assembler that reads the same sparse `StructuredMoA` GG surface and writes JSON/Markdown/SVG outputs for the curved wraparound whip geometry view.
- `ch17-hetero-moa-fabric-curvature-figure.ts`: supplemental topology artifact assembler that writes JSON/Markdown/SVG outputs for the backend-diverse `HeteroMoAFabric` curvature view, carrying the same wraparound whip geometry into CPU/GPU/NPU/WASM layer racing.
