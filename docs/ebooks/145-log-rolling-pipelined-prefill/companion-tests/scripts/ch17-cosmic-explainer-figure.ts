...
function parseCli(argv: readonly string[]): CliOptions {
  ...
  const defaultJsonPath = resolve(
    moduleDir,
    '../artifacts/ch17-cosmic-explainer-figure.json'
  );
  const defaultMarkdownPath = resolve(
    moduleDir,
    '../artifacts/ch17-cosmic-explainer-figure.md'
  );
  const defaultSvgPath = resolve(
    moduleDir,
    '../artifacts/ch17-cosmic-explainer-figure.svg'
  );
...
async function main(): Promise<void> {
  ...
  const spec = getChapter17SceneSpec('cosmic-explainer');
  const verifiedSpec = options.assertSurface
    ? assertCosmicSceneSurface(spec)
    : spec;
