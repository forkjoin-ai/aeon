...
function parseCli(argv: readonly string[]): CliOptions {
  ...
  const defaultJsonPath = resolve(
    moduleDir,
    '../artifacts/gnosis-fold-training-benchmark.json'
  );
  const defaultMarkdownPath = resolve(
    moduleDir,
    '../artifacts/gnosis-fold-training-benchmark.md'
  );
  ...
  for (const strategy of report.rankingByEvalMeanSquaredError) {
    ...
    process.stdout.write(
      `- ${strategy}: eval-mse=${metrics.meanEvalMeanSquaredError.toFixed(
        ...
        3
      )}, cancel=${metrics.meanCancellationLineMeanAbsoluteError.toFixed(3)}\n`
    );
  }
