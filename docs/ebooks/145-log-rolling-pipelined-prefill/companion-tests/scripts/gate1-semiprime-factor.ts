...
function parseCli(argv: readonly string[]): CliOptions {
  ...
  const defaultJsonPath = resolve(
    moduleDir,
    '../artifacts/gate1-wallclock-semiprime-factor.json',
  );
  const defaultMarkdownPath = resolve(
    moduleDir,
    '../artifacts/gate1-wallclock-semiprime-factor.md',
  );
...
async function main(): Promise<void> {
  ...
  const report = await runGate1Matrix(config, {
    label: options.label ?? 'loopback-semiprime-factor',
  });
  ...
  process.stdout.write(`gate1-semiprime verdict: ${report.gate.pass ? 'PASS' : 'DENY'}\n`);
