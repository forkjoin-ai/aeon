...
export class BatchTimingOptimizer {
    ...
    findOptimalWindow() {
        ...
        const avgLatency = recentMeasurements.reduce((sum, m) => sum + m.latencyMs, 0) /
            recentMeasurements.length;
        const avgBandwidth = recentMeasurements.reduce((sum, m) => sum + m.bandwidthMbps, 0) /
            recentMeasurements.length;
    ...
    getSchedulingDecision(batchSize, batchPriority = 'normal', isUserTriggered = false) {
        ...
        else if (currentWindow.quality === 'excellent' ||
        ...
        const estimatedDeliveryMs = (batchSize / ((currentWindow.bandwidthMbps * 1024 * 1024) / 8)) * 1000 +
    ...
    applyScheduling(batchSize, sendNow, actualDelay) {
        ...
        const totalWait = this.stats.averageWaitTimeMs * (this.stats.totalBatches - 1) +
            actualDelay;
