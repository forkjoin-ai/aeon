...
export function createProjectionArtifactId(buildHash, nodes) {
    return `projection:${stableContentHash(`${buildHash}:${nodes.map((node) => node.contentHash).join(':')}`)}`;
}
...
export function computeProjectionCacheKey(artifact, signature) {
    return stableContentHash(`${artifact.artifactId}:${artifact.buildHash}:${requestSignatureKey(signature)}`);
}
export function matchesRequestGuard(guard, signature) {
    ...
    if (guard.deviceBuckets?.length &&
        !guard.deviceBuckets.includes(normalized.deviceBucket)) {
        return false;
    }
    if (guard.viewportBuckets?.length &&
